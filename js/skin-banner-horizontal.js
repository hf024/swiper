...
$(function(){
	...
	var skinSwiper = new Swiper('.swiper-container',{
        ...
        //分页器pagination
        pagination: '.swiper-pagination', //定义一个Swiper的分页器。默认会在这个分页器里面生成与slide对应的span标签
        paginationClickable: true,//值为true时，点击分页器的指示点时会发生Swiper
        paginationAsRange: true, /*默认true，会在可视的slide对应的pagination按钮上
                            增加一个‘swiper-visible-switch’类名，
                            当你的pagination用于指示carousel模式的Swiper时很有用。
                            默认类名可通过paginationVisibleClass修改。*/
        createPagination: true, /*生成pagination。
            默认为true，Swiper会在pagination生成与slides数量相同的span元素
            。每一个span都有一个"swiper-pagination-switch"类名，
            活动的slide对应的span还有一个"swiper-active-switch"的类名，
            可视的slide对应的span还有一个"swiper-visible-switch"的类名。
            设置为false，则不生成这些span。*/

        wrapperClass: 'swiper-wrapper', //设置wrapper的css类名
        slideClass: 'swiper-slide' , //设置slide的类名
        paginationElement: 'a', //pagination分页器内元素的html标签
        ...
        onSlideChangeStart: function(swiper,even){
            $('.swiper-slide-active').mouseover(function (e){
                $('.swiper-navigation-inner').addClass('swiper-navigation-hover');
            });

            $('.swiper-slide-active').mouseout(function(e){
                $('.swiper-navigation-inner').removeClass('swiper-navigation-hover');
            });
        }
    })
